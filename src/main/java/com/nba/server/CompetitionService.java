...
public class CompetitionService {
     ...
     * @return
     */
    public Long countCompetition(Integer gameId) {
        ...
        List<Competition> competitionList = competitionDAO.selectByExample(competitionExample);
        if (!competitionList.isEmpty()) {
            Integer competitionId = competitionList.get(0).getCompetitionId();
            CompetitionListExample competitionListExample = new CompetitionListExample();
            CompetitionListExample.Criteria criteria = competitionListExample.createCriteria();
            criteria.andCompetitionIdEqualTo(competitionId);
            criteria.andUidEqualTo((int) request.getSession().getAttribute("userId"));
            return competitionListDAO.countByExample(competitionListExample);
        } else {
            return (long) 0;
        }
    }
